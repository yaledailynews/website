...
export async function SmallHeader() {
  ...
  return (
    <header className="px-3 py-2 bg-black text-white grid grid-cols-3 items-center sticky top-0 z-50 shadow">
      <input type="checkbox" id="drawer-toggle" className="hidden" />
      <div className="flex justify-start items-center">
        <label htmlFor="drawer-toggle" className="drawer-toggle-label cursor-pointer p-2">
          <IconMenu2 className="size-5" />
        </label>
      </div>
      <Link href="/" className="flex justify-center items-center">
        <Image src={logo} alt="Yale Daily News" className="max-w-36 h-auto invert" />
      </Link>
